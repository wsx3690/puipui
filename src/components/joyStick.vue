...
export default {
  ...
  mounted () {              //Lifecycle of vue   
    ...
    const staticGamepad = nipplejs.create({        //set joystick on web
    ...
    staticGamepad
      .on('dir:up dir:left dir:down dir:right', this.staticGamepadMove)    /* 綁定搖桿事件處理 事件分別為上 下 左 右，用 this.staticGamepadMove publish 值到 broker。第二個參數就是處理的函數(定義在methods 裡面) */
